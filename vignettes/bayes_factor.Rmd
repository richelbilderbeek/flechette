...
```{r}
mcmc <- beautier::create_mcmc(chain_length = 50000)
...
Calculate the Bayes factor (usually denoted as K). 
The Bayes factor equals the likelihood odds iff both models are equally likely.
From an MCMC run, we can simply take the average likelihood [1]:

```{r}
bayes_factor <- mean(pos_jc69$likelihood) / mean(pos_gtr$likelihood)
print(bayes_factor)
```
...
* [1] Stack Exchange: https://stats.stackexchange.com/a/204724
